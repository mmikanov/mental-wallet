...
| [add-your-own-app](#rec-add-your-own-app) | Screen-recorded video (Create Tool → Link Button flow) | **You** (recording) + **Agent** (clean/GIF/wire) | ✅ Done — live (web inline GIF + email/feed hero). Finch example: wallet → ⋮ (pulse) → Create Tool (pulse) → Step 1 shell → Step 2 empty → +Add block (pulse) → Link Button (pulse) → URLs → Step 3 Preview → Save → wallet with new card → expanded card + Open button (pulse) → fade. | `ccffd297-777d-4c00-a0ed-8e7520d74556` (draft) |
| [add-your-own-tool](#rec-add-your-own-tool) | Screen video (Create Tool → input blocks → use it) | **You** (recording) + **Agent** (clean/GIF/wire) | ✅ Done — live (web inline GIF + email/feed hero). Minimal edit of the real recording (variable speed + fade); slowed the Create Tool menu + both block-picker moments; amber pulse on Text Input then Mood Slider. wallet → ⋮ → Create Tool → Step 1 shell → Step 2 (add Text Input + Mood Slider blocks) → Save → wallet → open + use the finished tool. | `088229be-082c-496d-bca9-499c2e1ea812` (draft) |
| [reorder-tools](#rec-reorder-tools) | Short screen video (long-press → Reorder Cards → ▲/▼ → Done) | **You** (recording) + **Agent** (clean/GIF/wire) | ✅ Done — live (web inline GIF + email/feed hero). Wallet (amber pulse hint on top card) → Reorder Cards panel → ▲/▼ moves → Done → wallet with new order → fade. | `1eca54e9-8e73-4ab5-88b9-a4fac52f8f9b` (draft) |
| [archive-restore-tools](#rec-archive-restore-tools) | Screen video (⋮ → Archive card → Archive screen → Restore) | **You** (recording) + **Agent** (clean/GIF/wire) | ✅ Done — live (web inline GIF + email/feed hero). Minimal edit of the real recording (trim+speed+fade); single amber pulse on Archive in the wallet ⋮ menu (the only hard-to-see tap). Focus card → ⋮ → Archive card → confirm → wallet → ⋮ → Archive → Archive screen → Restore to Wallet → confirm → wallet with card back. | `09196fdf-bfeb-49c2-933e-a33d7c90c78b` (draft) |
