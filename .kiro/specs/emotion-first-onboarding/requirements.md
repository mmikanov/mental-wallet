...
8. THE Session_Launcher_Card SHALL be visually distinguishable from other cards (e.g., unique icon or accent color) so the user can identify it at a glance in the card stack.
...
1. WHEN the user taps a tool card preview in the Session_View, THE app SHALL navigate to that card's full expanded view (same layout as the standard card expansion in the Wallet_Screen), presented as a sub-screen within the session context.
...
4. WHEN the user returns from a tool in the "Suggested tools to try" section, THE Session_View SHALL display an "Add to wallet" option for that tool, positioned below the tool's card preview.
5. WHEN the user taps "Add to wallet" for a suggested tool, THE app SHALL add the tool card to the user's wallet with origin badge "library," persist the card to the database, display a brief confirmation (e.g., toast or inline checkmark), and move the tool from the "Suggested tools to try" section to the "From your wallet" section in the current Session_View. IF any step in this operation fails (e.g., database persistence fails), THEN THE app SHALL abort the entire operation, leave the tool in the "Suggested tools to try" section, and display an error message.
...
2. WHEN the user taps "End session," THE app SHALL mark the Emotion_Session as ended, record the end timestamp, and navigate back to the Wallet_Screen.
