...
1. THE App SHALL support logging the following event types and no others: `app_opened`, `onboarding_step_viewed`, `onboarding_intent_selected`, `onboarding_kpi_selected`, `onboarding_completed`, `start_mode_selected`, `session_started`, `tool_added`, `tool_archived`, `tool_unarchived`, `tool_created`, `tool_copied`, `tool_opened`, `tool_completed`, `tool_history_viewed`, `reminder_set`, `reminder_deleted`, `outcome_response`, `external_resource_opened`, and `session_ended`.
...
3. WHEN an `onboarding_step_viewed` event is logged, THE App SHALL include a `step_name` property (TEXT, max 100 characters) containing the programmatic screen identifier of the onboarding screen viewed. THE App SHALL log this event each time the screen gains focus (including back-navigation), not only on initial mount.
3a. WHEN an `onboarding_intent_selected` event is logged, THE App SHALL include an `intent_id` property (TEXT, one of: `overwhelm`, `routine`, `organize`, `explore`) indicating which intent the user selected.
3b. WHEN an `onboarding_kpi_selected` event is logged, THE App SHALL include a `kpi_label` property (TEXT, one of the predefined KPI labels or `custom` for user-written values). THE App SHALL NOT log the user's free-text input — only the string `custom` to indicate a custom KPI was chosen.
